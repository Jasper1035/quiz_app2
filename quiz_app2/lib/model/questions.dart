import 'package:quiz_app2/model/question.dart';

final List<Question> questions = [
  Question(
    question: '1. What is the capital of France?',
    correctAnswerIndex: 1,
    options: const ['a) Madrid', 'b) Paris', 'c) Berlin', 'd) Rome'],
  ),
  Question(
    question: '2. In what continent is Brazil located?',
    correctAnswerIndex: 3,
    options: const [
      'a) Europe',
      'b) Asia',
      'c) North America',
      'd) South America',
    ],
  ),
  Question(
    question: '3. What is the largest planet in our solar system?',
    correctAnswerIndex: 1,
    options: const ['a) Earth', 'b) Jupiter', 'c) Saturn', 'd) Venus'],
  ),
  Question(
    question: '4. What is the longest river in the world?',
    correctAnswerIndex: 0,
    options: const ['a) Nile', 'b) Amazon', 'c) Mississippi', 'd) Danube'],
  ),
  Question(
    question: '5. Who is the main character in the Harry Potter series?',
    correctAnswerIndex: 2,
    options: const [
      'a) Hermione Granger',
      'b) Ron Weasley',
      'c) Harry Potter',
      'd) Neville Longbottom',
    ],
  ),
  Question(
    question: '6. What is the smallest planet in our solar system?',
    correctAnswerIndex: 3,
    options: const ['a) Venus', 'b) Mars', 'c) Earth', 'd) Mercury'],
  ),
  Question(
    question: '7. Who wrote the play Romeo and Juliet?',
    correctAnswerIndex: 0,
    options: const [
      'a) William Shakespeare',
      'b) Oscar Wilde',
      'c) Jane Austen',
      'd) Charles Dickens',
    ],
  ),
  Question(
    question: '8. What is the highest mountain in the world?',
    correctAnswerIndex: 1,
    options: const [
      'a) Mont Blanc',
      'b) Everest',
      'c) Kilimanjaro',
      'd) Aconcagua',
    ],
  ),
  Question(
    question:
        '9. What is the name of the famous painting by Leonardo da Vinci that depicts a woman?',
    correctAnswerIndex: 3,
    options: const [
      'a) Starry Night',
      'b) The Persistence of Memory',
      'c) The Last Supper',
      'd) Mona Lisa',
    ],
  ),
];
